...
(async()=>{
  ...
  for(const viewport of [{width:390,height:844,name:'mobile'},{width:1440,height:900,name:'desktop'}]){
    ...
    if(await page.locator('#tablePreviewTitle').innerText() !== 'Столик 16') throw new Error('Wrong table photo title');
    ...
    if(!await page.locator('#tablePreviewImage').getAttribute('src').then(src => src.endsWith('table-16.jpg'))) throw new Error('Wrong table photo');
    ...
    await page.waitForFunction(() => document.querySelector('#tablePreviewImage')?.alt === 'Зал HHBAR');
    if(!await page.locator('#tablePreviewText').innerText().then(text => text.includes('Фото именно этого столика пока нет'))) throw new Error('Missing photo fallback is unclear');
    await page.locator('#tablePreviewModal [data-restoplace-booking]').click();
    await page.waitForFunction(() => window.__restoplaceOpens === 3);
    if(await page.evaluate(() => window.__restoplaceItem) !== '830818') throw new Error('Table 15 did not retain its booking ID after photo fallback');
